...
/**
 * Panel de operador de horno: Órdenes activas en producción para registro rápido en planta.
 */
@Component({
  selector: 'app-horno-lista',
      ...
      <header class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div class="flex items-center gap-2">
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900">Operación de Hornos</h1>
          ...
          <p class="mt-1 text-sm text-slate-500 font-medium">
            Selecciona la orden activa en tu línea para registrar bultos crudos, temperaturas y niveles de aceite.
          </p>
        </div>
      </header>
      ...
      @if (cargando()) {
        ...
        <app-tarjeta [sinRelleno]="true">
          <app-estado-vacio
            ...
            descripcion="Cuando el supervisor autorice e inicie una orden en tu línea de fritura, aparecerá aquí inmediatamente."
          />
        </app-tarjeta>
      } @else {
            ...
            <div class="group relative flex flex-col justify-between rounded-3xl border border-slate-200/90 bg-white p-6 shadow-xs transition-all duration-200 hover:border-slate-300 hover:shadow-md">
              <div>
                ...
                <div class="flex items-center justify-between gap-2 mb-3">
                  <span class="inline-flex items-center gap-1.5 rounded-xl bg-marca-50 px-3 py-1 text-xs font-extrabold text-marca-800 border border-marca-200/60">
                    <span>♨️</span> {{ orden.horno.nombre }}
                  </span>
  ...
  protected tomar(orden: Orden): void {
    ...
    this.servicio.iniciar(orden.id).subscribe({
      next: () => {
        ...
        this.avisos.exito(`Orden ${orden.numero_orden} iniciada. Ya puedes registrar tandas.`);
        this.cargar();
      },
